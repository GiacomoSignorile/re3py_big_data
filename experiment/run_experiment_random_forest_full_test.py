"""
Re3py Data Scarcity Experiment - Random Forest Full Test
=========================================================

Main experiment runner for evaluating Re3py Random Forest on reduced datasets,
while testing on the full (original) test folds.

Configuration:
- Model: Random Forest
- Feature Sampling: sqrt(F) features evaluated at each node (Section 5.3, page 11)
- Validation: 10-fold Cross-Validation
- Metrics:
  - Classification: Accuracy, Precision, Recall, F1
- Data Reductions: 0%, 10%, 20%, 50%, 70%, 90% (incremental)
...
This implementation follows the paper exactly: like Bagging but with feature subsampling.
The authors set the number of features to sample equal to sqrt(total_features).
"""
...
class DataScarcityExperiment:
    """Runs data scarcity experiment with Re3py Random Forest."""
    ...
    def _setup_logger(self):
        ...
        self.logger = logging.getLogger(f"experiment_rf_full_test_{self.dataset_name}")
        ...
        # File handler
        log_file = logs_dir / f"random_forest_full_test_{self.dataset_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def _run_single_fold(
        ...
        Args:
            fold_idx: Fold index
            train_data: Training dataset (already filtered to reduced IDs)
            test_data: Test dataset
            full_classes: List of all target classes
            n_folds: Total number of folds (for logging)
            n_estimators: Number of trees in random forest

        Returns:
            Tuple of (fold_result_dict, test_set_size)
        """
        ...
        try:
            ...
            # Calculate feature sampling: F' = sqrt(F) as per paper (Section 5.3, page 11)
            all_aggregators = settings.get_aggregates()
            num_features = len(all_aggregators)
            sqrt_features = max(1, int(math.sqrt(num_features)))  # At least 1, rounded down
            max_relative_features = sqrt_features / num_features
            
            self.logger.debug(f"Fold {fold_idx + 1}: Feature sampling - Total features: {num_features}, sqrt(F): {sqrt_features}, Relative: {max_relative_features:.4f}")
            
            tree_params = {
                "allowed_atom_tests": settings.get_atom_tests_structured(),
                "allowed_aggregators": all_aggregators,
                "minimal_examples_in_leaf": 1,  # Fully grown trees (Section 5.3, page 11)
                "java_port": None,
                "per_class_bootstrap": True,  # Bootstrap aggregates
                "only_existential": self._get_only_existential_flag(),
                "max_relative_number_of_evaluated_tests_per_node": max_relative_features,  # Key RF difference: sqrt(F)/F
                "longest_atom_test_chain": 2,  # Look-ahead depth = 2 (Section 5.3, page 11)
            }

            rf_model = RandomForest(
                nb_trees_to_build=n_estimators,  # 50 trees (Section 5.3, page 11)
            ...
            self.logger.debug("Starting Random Forest build for fold %s", fold_idx + 1)
            ...
            self.logger.info(
                "Fold %s/%s completed - Accuracy: %.4f, Precision: %.4f, Recall: %.4f, F1: %.4f",
                fold_idx + 1,
                n_folds,
    ...
    def run_random_forest_cv(
        ...
        """
        Run Random Forest with CV using reduced training and full test set.
        ...
        """
        print(f"\n{'=' * 70}")
        print(f"Running Random Forest on {percentage}% reduced data (full test)")
        print(f"{'=' * 70}")

        fold_results = []
        test_sizes = []
        self.logger.info(f"Starting Random Forest on {percentage}% reduced data (full test)")
        ...
        try:
            ...
            all_folds = list(
                enumerate(create_folds(original_dataset, example_ids=folds_list))
            )
            ...
            fold_results_with_sizes = Parallel(n_jobs=self.n_jobs, verbose=10)(
                ...
                for fold_idx, (full_train_data, test_data) in all_folds
            )
    ...
    def _load_checkpoint(self) -> bool:
        ...
        """
        summary_file = self.log_dir / f"{self.dataset_name}_summary_random_forest_{self.config_name}_full_test.json"
    ...
    def _reduction_already_completed(self, percentage: int) -> bool:
        ...
        Returns:
            True if results exist for this reduction, False otherwise
        """
    ...
    def _save_checkpoint(self) -> None:
        ...
        # Also update CSV with current results
        csv_file = self.log_dir / f"{self.dataset_name}_results_random_forest_{self.config_name}_full_test.csv"
    ...
    def run_all_reductions(self) -> Dict[str, Any]:
        ...
        print(f"{'=' * 70}")
        print("Model: Random Forest (feature sampling = sqrt(F), full test)")
        ...
        for percentage in self.reduction_percentages:
            ...
            try:
                ...
                results = self.run_random_forest_cv(
                    reduced_dataset,
                    original_dataset,
                    folds_file,
                    percentage,
    ...
    def save_results(self) -> None:
        ...
        detailed_file = self.log_dir / f"{self.dataset_name}_detailed_random_forest_{self.config_name}_full_test.json"
        detailed_results = {
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run data scarcity experiment with Re3py Random Forest on reduced datasets (full test)."
    )
    ...
    for dataset in datasets:
        try:
            ...
            logging.getLogger(f"experiment_rf_full_test_{dataset}").error(
                f"Error running experiment: {e}", exc_info=True
            )
            traceback.print_exc()
            continue
