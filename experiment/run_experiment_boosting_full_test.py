...
class DataScarcityExperiment:
    ...
    def _setup_logger(self):
        ...
        self.logger = logging.getLogger(f"experiment_full_test_{self.dataset_name}_gb")
        self.logger.setLevel(logging.DEBUG)

        logs_dir = self.base_dir / "logs"
        logs_dir.mkdir(parents=True, exist_ok=True)

        log_file = logs_dir / f"boosting_full_test_{self.dataset_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def _run_single_fold(
        ...
        try:
            ...
            try:
                ...
                if build_output:
                    self.logger.debug("Tree building output for fold %s:\n%s", fold_idx + 1, build_output)
    ...
    def run_gradient_boosting_cv(
        ...
        try:
            ...
            for fold_result, test_size in fold_results_with_sizes:
                ...
                print(
                    "Fold %s - Acc: %.4f | Prec: %.4f | Rec: %.4f | F1: %.4f"
                    % (
                        fold_result["fold"] + 1,
                        fold_result["accuracy"],
                        fold_result["precision"],
    ...
    def run_all_reductions(self) -> Dict[str, Any]:
        ...
        for percentage in self.reduction_percentages:
            ...
            if self._reduction_already_completed(percentage):
                print(f"⊘ Skipping {percentage}% reduction (already completed)")
                continue
