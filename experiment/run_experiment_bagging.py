"""
Re3py Data Scarcity Experiment - Bagging 
==================================================
...
class DataScarcityExperiment:
    ...
    def __init__(self, dataset_name: str, log_dir: Path = None, use_original_folds: bool = False):
        """
    ...
    def run_bagging_cv(self, dataset: Dataset, folds_file: Path,
                      ...
                      n_estimators: int = 50, shrinkage: float = 0.1) -> Dict[str, Any]:
        """
        Run GradientBoosting with CV on reduced dataset.
        ...
        """
        print(f"\n{'='*70}")
        print(f"Running GradientBoosting on {percentage}% reduced data")
        print(f"{'='*70}")
    ...
    def run_all_reductions(self) -> Dict[str, Any]:
        ...
        print(f"Metrics: Accuracy, Precision, Recall, F1, MSE, MAE, RMSE")
    ...
    def save_results(self) -> None:
        ...
        with open(csv_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['dataset', 'reduction_%', 
            ...
            for reduction_pct, metrics in self.results['results_by_reduction'].items():
                writer.writerow([
                    ...
                    metrics.get('mse_mean', 0),
                    metrics.get('mse_std', 0),
                    metrics.get('mae_mean', 0),
                    metrics.get('mae_std', 0),
                    metrics.get('rmse_mean', 0),
                    metrics.get('rmse_std', 0)
                ])
...
def main():
    ...
    parser.add_argument('--use-original-folds', action='store_true',
    ...
    for dataset in datasets:
        try:
            experiment = DataScarcityExperiment(dataset, args.log_dir, use_original_folds=args.use_original_folds)
