"""
Re3py Data Scarcity Experiment - Bagging Full Test
==================================================
...
class DataScarcityExperiment:
    ...
    def _setup_logger(self):
        ...
        self.logger = logging.getLogger(f"experiment_full_test_{self.dataset_name}")
        ...
        # File handler
        log_file = logs_dir / f"bagging_full_test_{self.dataset_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def _run_single_fold(
        ...
        try:
            ...
            self.logger.debug("Starting RandomForest build for fold %s", fold_idx + 1)
    ...
    def run_bagging_cv(
        ...
        """
        Run Random Forest (Bagging) with CV using reduced training and full test set.
        ...
        """
        print(f"\n{'=' * 70}")
        print(f"Running Bagging on {percentage}% reduced data (full test)")
        print(f"{'=' * 70}")

        fold_results = []
        test_sizes = []
        self.logger.info(f"Starting Bagging on {percentage}% reduced data (full test)")
    ...
    def _load_checkpoint(self) -> bool:
        ...
        """
        csv_file = self.log_dir / f"{self.dataset_name}_results_bagging_{self.config_name}_full_test.csv"
        summary_file = self.log_dir / f"{self.dataset_name}_summary_bagging_{self.config_name}_full_test.json"
    ...
    def save_results(self) -> None:
        ...
        detailed_file = self.log_dir / f"{self.dataset_name}_detailed_bagging_{self.config_name}_full_test.json"
        detailed_results = {
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run data scarcity experiment with Re3py Bagging on reduced datasets (full test)."
    )
    ...
    for dataset in datasets:
        try:
            ...
            logging.getLogger(f"experiment_full_test_{dataset}").error(
