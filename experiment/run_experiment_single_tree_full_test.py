"""
Re3py Data Scarcity Experiment - Single Tree Full Test
======================================================
...
class DataScarcityExperiment:
    ...
    def _setup_logger(self):
        ...
        self.logger = logging.getLogger(f"experiment_full_test_{self.dataset_name}_st")
        ...
        # File handler
        log_file = logs_dir / f"single_tree_full_test_{self.dataset_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def _run_single_fold_full_test(
        self,
        fold_idx: int,
        ...
        """
        Run a single fold of single tree training (full test variant).

        Args:
            fold_idx: Fold index
            full_train_data: Full training dataset (before filtering)
            test_data: Test dataset
            full_classes: List of all possible classes
            n_folds: Total number of folds
            reduced_ids: Set of IDs to keep in training data

        Returns:
            Tuple of (fold_result dict, test_size)
        """
        # Filter training data to reduced dataset IDs
        ...
        try:
            ...
            self.logger.info(f"Fold {fold_idx + 1}/{n_folds}: Best params - leaf_size={best_leaf_size}, impurity={best_impurity:.4f}, inner_cv_acc={inner_cv_score:.4f}")
    ...
    def run_single_tree_cv(
        ...
        """
        Run Single Decision Tree with CV using reduced training and full test set.
        ...
        """
        print(
            "\nRunning Single Tree on %s%% reduced data (full test)"
            % percentage
        )

        fold_results = []
        test_sizes = []
        self.logger.info(f"Starting Single Tree on {percentage}% reduced data (full test)")
        ...
        try:
            ...
            fold_results_with_sizes = Parallel(n_jobs=self.n_jobs, verbose=10)(
                delayed(self._run_single_fold_full_test)(
                    fold_idx=fold_idx,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run data scarcity experiment with Re3py Single Tree on reduced datasets (full test)."
    )
    ...
    for dataset in datasets:
        try:
            ...
            logging.getLogger(f"experiment_full_test_{dataset}_st").error(
