"""
Re3py Data Scarcity Experiment - Random Forest
===============================================
...
Note: The paper (Table 4, page 13) shows Bagging actually outperformed RF on most datasets,
suggesting that in a relational context, sampling too few features might miss important aggregates.
"""
...
class DataScarcityExperiment:
    ...
    def _setup_logger(self):
        ...
        self.logger = logging.getLogger(f"experiment_rf_{self.dataset_name}")
        ...
        # File handler
        log_file = logs_dir / f"random_forest_{self.dataset_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def _run_single_fold(
        ...
        """
        Run a single fold of Random Forest.
        ...
        try:
            ...
            self.logger.debug(f"Starting Random Forest build for fold {fold_idx + 1}/{n_folds}")
    ...
    def run_random_forest_cv(
        ...
        """
        Run Random Forest with CV on reduced dataset.
    ...
    def _load_checkpoint(self) -> bool:
        ...
        summary_file = self.log_dir / f"{self.dataset_name}_summary_random_forest_{self.config_name}.json"
    ...
    def _save_checkpoint(self) -> None:
        ...
        csv_file = self.log_dir / f"{self.dataset_name}_results_random_forest_{self.config_name}.csv"
    ...
    def run_all_reductions(self) -> Dict[str, Any]:
        ...
        print(f"{'=' * 70}")
        print("Model: Random Forest (feature sampling = sqrt(F))")
        ...
        for percentage in self.reduction_percentages:
            ...
            try:
                ...
                results = self.run_random_forest_cv(dataset, folds_file, percentage)
    ...
    def save_results(self) -> None:
        ...
        detailed_file = self.log_dir / f"{self.dataset_name}_detailed_random_forest_{self.config_name}.json"
        detailed_results = {
...
def main():
    ...
    for dataset in datasets:
        try:
            ...
            logging.getLogger(f"experiment_rf_{dataset}").error(f"Error running experiment: {e}", exc_info=True)
            traceback.print_exc()
            continue
